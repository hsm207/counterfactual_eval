...
class Experiment:
    def __init__(self, dataset, ranking_size):
        ...
        print(
            "Experiment:init [INFO] Dataset: %s RankingSize: %d"
            % (dataset.name, ranking_size),
            flush=True,
        )
    ...
    def setupLogger(self, out_dir, det_policy, temperature):
        ...
        fileName = (
            out_dir + loggingPolicy.name() + "_gamma" + str(self.rankingSize) + ".pkl"
        )
        ...
        print("Experiment:setupLogger [INFO] Temperature: %f" % temperature, flush=True)
    ...
    def setupMetric(self, metric_string, dataset):
        ...
        if metric_string == "Constant":
            ...
            print(
                "Experiment:setupMetric [ERR] Metric ",
                metric_string,
    ...
    def setupEstimators(self, estimator_list):
        ...
        for i in range(numEstimators):
            ...
            if tokens[0] == "Direct":
                currentEstimator = Estimators.EstimatorWrapper(
                    Estimators.DirectEstimator(
                        self.logger.policy, tokens[1], self.rankingSize, numFeatures
                    )
                )
            elif tokens[0] == "IPS":
                currentEstimator = Estimators.EstimatorWrapper(
                    Estimators.InversePropensityEstimator(
                        self.rankingSize, self.logger.policy
                    )
                )
            elif tokens[0] == "IPS-SN":
                currentEstimator = Estimators.SelfNormalWrapper(
                    Estimators.InversePropensityEstimator(
                        self.rankingSize, self.logger.policy
                    )
                )
                ...
                currentEstimator = Estimators.EstimatorWrapper(
                    Estimators.GammaEstimator(self.rankingSize, self.logger.policy)
                )
                ...
                ipsEstimator = Estimators.InversePropensityEstimator(
                    self.rankingSize, self.logger.policy
                )
                directEstimator = Estimators.DirectEstimator(
                    self.logger.policy, tokens[1], self.rankingSize, numFeatures
                )

                currentEstimator = Estimators.EstimatorWrapper(
                    Estimators.DoublyRobustEstimator(ipsEstimator, directEstimator)
                )
            else:
                print(
                    "Experiment:setupEstimators [ERR] %s currently not supported."
                    % estimatorName
                )
                sys.exit(0)
    ...
    def trainEstimators(
        self, explored_queries, new_rankings, explored_ranking_features, explored_values
    ):
        ...
        for trainEstimator, estimatorWrap in self.estimators.items():
            ...
            if prefix == "DR":
                estimator.directEstimator.train(
                    folds, explored_ranking_features, explored_values
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max_docs", "-m", metavar="M", type=int, help="Filter documents", default=7
    )
    parser.add_argument(
        "--length_ranking", "-l", metavar="L", type=int, help="Ranking Size", default=3
    )
    parser.add_argument(
        "--temperature",
        "-t",
        metavar="T",
        type=float,
        help="Temperature for logging policy",
        default=1.0,
    )
    parser.add_argument(
        "--filtering_ranker",
        ...
        metavar="F",
        type=str,
        help="Model for filtering ranker",
        default="lasso",
    )
    parser.add_argument(
        "--evaluation_ranker",
        ...
        metavar="E",
        type=str,
        help="Model for evaluation ranker",
        default="lasso",
    )
    parser.add_argument(
        "--value_metric",
    ...
    parser.add_argument(
        ...
        metavar="S",
        type=int,
        help="How many times expt is repeated",
        default=10,
    )
    parser.add_argument(
        "--numpy_seed",
    ...
    if args.max_docs is not None:
        ...
        filterer.train(
            dataset=data, feature_list=anchorTitleURLFeatures, exponential_gains=False
        )
    ...
    if filterer is not None:
        ...
        for i in range(numQueries):
            ...
            positionMatches = (
                (filterRankings[i, 0:validDocs] - newRankings[i, 0:validDocs]) == 0
        ...
        print(
            "Discrepancy between filterer and target: ExactMatches: %0.3f, PerPositionMatches:"
            % perfectMatches,
            matchesPerPosition,
            flush=True,
        )
    ...
    saveValues = numpy.linspace(
        9, numInstances - 1, num=saveInstances, endpoint=True, dtype=numpy.int32
    )
    ...
    outputString += (
        "f"
        + args.filtering_ranker
        + "_e"
        + args.evaluation_ranker
        + "_"
        + str(args.numpy_seed)
    )
    ...
    for run in range(numRuns):
        ...
        exploredRankingFeatures = numpy.empty(
            (maxTrainInstances, numFeatures * args.length_ranking), dtype=numpy.float32
        )
        ...
        for i in range(len(BreakPoints)):
            ...
            if i != 0:
                expt.trainEstimators(
                    exploredQueries[0:startInstance],
                    newRankings,
                    exploredRankingFeatures[0:startInstance, :],
            ...
            for j in range(startInstance, endInstance):
                currentQuery, currentRanking, currentValue = expt.logger.createOneSample(
                    expt.metric
                )
                ...
                if i < (len(BreakPoints) - 1):
                    rankedDocFeatures = numpy.zeros(
                        (args.length_ranking, numFeatures), dtype=numpy.float32
                    )
                    rankedDocFeatures[0:validDocs, :] = expt.dataset.features[
                        currentQuery, currentRanking[0:validDocs], :
                    ]
                    exploredRankingFeatures[j, :] = numpy.reshape(
                        rankedDocFeatures, numFeatures * args.length_ranking
                    )
                ...
                for estimatorName, estimator in expt.estimators.items():
                    estimatedValue = estimator.estimate(
                        currentQuery, currentRanking, currentValue, newRanking
                    )
