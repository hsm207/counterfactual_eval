...
def plot_mse_stderr(PKL, approach_list, plot_estimate, color_string="bmgrcky"):
    ...
    for approachIndex, approach in enumerate(approach_list):
        ...
        if "Direct" in approach:
            ...
            snipPoint = np.where(x == limit)[0][0]
        ...
        legendHandles.append(
            matplotlib.patches.Patch(color=currColor, label=prefix + approachName)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", action="store", help="pkl file path with data")
