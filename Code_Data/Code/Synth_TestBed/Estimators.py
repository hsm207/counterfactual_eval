...
class DirectEstimator(TrainedEstimator):
    ...
    def __init__(self, policy, estimator_type, ranking_size, num_features):
        TrainedEstimator.__init__(
            self, ranking_size, policy, num_features * ranking_size
        )
    ...
    def saveModel(self, file_path, num_instances):
        ...
        outFile.close()
        print(
            "DirectEstimator:saveModel [INFO] Saved Model after %d samples at"
            % num_instances,
            outputFileName,
            flush=True,
        )
    ...
    def loadModel(self, file_name):
        ...
        print(
            "DirectEstimator:loadModel [INFO] Loaded trained model for direct estimator from ",
            file_name,
            flush=True,
        )
    ...
    def train(
        self,
        folds,
        explored_ranking_features,
        explored_values,
        inverse_propensities=None,
    ):
        ...
        if self.estimatorType == "tree":
            treeCV = sklearn.grid_search.GridSearchCV(
                ...
                scoring=None,
                fit_params={"sample_weight": inverse_propensities},
                n_jobs=1,
                pre_dispatch=1,
                iid=False,
                cv=folds,
            ...
            print(
                "DirectEstimator:train [INFO] %s Best depth: %d"
                % (self.estimatorType, treeCV.best_params_["max_depth"]),
                flush=True,
            )
            ...
            cv.fit(
                explored_ranking_features,
                explored_values,
                sample_weight=inverse_propensities,
            )
            self.params = cv.coef_
            print(
                "DirectEstimator:train [INFO] %s CVAlpha: %f"
                % (self.estimatorType, cv.alpha_),
                flush=True,
            )
            ...
            self.params = scipy.linalg.lstsq(
                explored_ranking_features, explored_values, cond=1e-9
            )[0]
            print("DirectEstimator:train [INFO] %s" % self.estimatorType, flush=True)
        else:
            print(
                "DirectEstimator:train [ERR] %s not supported." % self.estimatorType,
                flush=True,
            )
            sys.exit(0)
    ...
    def precompute(self, predicted_rankings):
        ...
        for i in range(numQueries):
            ...
            newRankingFeatures = numpy.zeros(
                (self.rankingSize, numFeatures), dtype=numpy.float32
            )
            newRankingFeatures[0:validDocs, :] = self.policy.dataset.features[
                i, predicted_rankings[i, 0:validDocs], :
            ]

            newRankingFeatures = numpy.reshape(newRankingFeatures, self.numParams)
            estimates[i] = self.estimateHelper(newRankingFeatures)
        print(
            "DirectEstimator:precompute [INFO] Precomputed ",
            numpy.shape(predicted_rankings),
            flush=True,
        )
...
class InversePropensityEstimator(Estimator):
    ...
    def estimate(self, query, explored_ranking, explored_value, new_ranking):
        ...
        if exactMatch:
            ...
            queryDocProbabilities = self.policy.documentProbabilities[
                query, 0:numAllowedDocs
            ]
...
class GammaEstimator(Estimator):
    ...
    def estimate(self, query, explored_ranking, explored_value, new_ranking):
        ...
        exploredMatrix = numpy.zeros(
            (validDocs, numAllowedDocs), dtype=numpy.longdouble
        )
...
class DoublyRobustEstimator(Estimator):
    ...
    def estimate(self, query, explored_ranking, explored_value, new_ranking):
        reward = self.directEstimator.estimate(
            query, explored_ranking, explored_value, new_ranking
        )

        ipsReward = self.ipsEstimator.estimate(
            query, explored_ranking, explored_value - reward, new_ranking
        )
...
class SelfNormalWrapper(EstimatorWrapper):
    ...
    def estimate(self, query, explored_ranking, explored_value, new_ranking):
        ...
        invPropensity = self.estimator.estimate(
            query, explored_ranking, 1.0, new_ranking
        )
...
if __name__ == "__main__":
    ...
    trainQueries, trainRankings, trainMetricValues = nonuniformLogger.createLog(
        numTrainInstances, revenue
    )
    exploredRankingFeatures = numpy.empty(
        (numTrainInstances, numFeatures * rankingSize), dtype=numpy.float32
    )
    for i in range(numTrainInstances):
        ...
        rankedDocFeatures[0:validDocs] = filteredData.features[
            currentQuery, currentRanking[0:validDocs], :
        ]
        exploredRankingFeatures[i, :] = numpy.reshape(
            rankedDocFeatures, numFeatures * rankingSize
        )
    ...
    for i in range(numInstances):
        currentQuery, exploredRanking, metricValues = nonuniformLogger.createOneSample(
            revenue
        )
        numAllowedDocs = filteredData.docsPerQuery[currentQuery]
        validDocs = min(numAllowedDocs, rankingSize)
        ...
        for estimator in estimators:
            currValue = estimator.estimate(
                currentQuery, exploredRanking, metricValues, producedRanking
            )
    ...
    for estimator in estimators:
        values[estimator.name] = (
            numpy.cumsum(values[estimator.name], dtype=numpy.longdouble) / denominators
        )
    ...
    for estimator in estimators:
        print(
            "%s Estimate=%f MSE=%0.5e"
            % (
                estimator.name,
                values[estimator.name][-1],
                (values[estimator.name][-1] - values["Truth"]) ** 2,
            ),
            flush=True,
        )
