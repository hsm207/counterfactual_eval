...
class Policy:
    ...
    def __init__(self, dataset):
        self.dataset = (
            dataset
        )  # This is the dataset to be used for predicting rankings.
...
class CustomPolicy(Policy):
    ...
    def createFeature(self, docFeatures, position):
        ...
        if self.crossFeatures:
            currFeature[
                self.numDocFeatures
                + self.rankingSize
                + position * self.numDocFeatures : self.numDocFeatures
                + self.rankingSize
                + (position + 1) * self.numDocFeatures
            ] = docFeatures
    ...
    def predict(self, query_id, ranking_size):
        ...
        for doc in range(allowedDocs):
            for pos in range(self.rankingSize):
                currFeature = self.createFeature(
                    self.dataset.features[query_id, doc, :], pos
                )
    ...
    def train(self, dataset, targets):
        ...
        designMatrix = numpy.zeros(
            (queryDocPairs * self.rankingSize, self.numFeatures), dtype=numpy.float32
        )
        regressionTargets = numpy.zeros(
            (queryDocPairs * self.rankingSize), dtype=numpy.float32
        )
        currID = -1
        for i in range(numQueries):
            ...
            for doc in range(numAllowedDocs):
                for j in range(self.rankingSize):
                    ...
                    designMatrix[currID, :] = self.createFeature(
                        dataset.features[i, doc, :], j
                    )
                    regressionTargets[currID] = targets[i, j, doc]

        if self.modelType == "tree":
            treeCV = sklearn.grid_search.GridSearchCV(
                sklearn.tree.DecisionTreeRegressor(presort=True),
            ...
            print(
                "CustomPolicy:train [INFO] %s Best depth: %d "
                % (self.modelType, treeCV.best_params_["max_depth"]),
                flush=True,
            )

        elif self.modelType == "ridge":
            cv = sklearn.linear_model.RidgeCV(
                alphas=self.hyperParams,
                fit_intercept=False,
                normalize=True,
                scoring=None,
            ...
            print(
                "CustomPolicy:train [INFO] %s CVAlpha: %f "
                % (self.modelType, cv.alpha_),
                flush=True,
            )

        elif self.modelType == "lasso":
            cv = sklearn.linear_model.LassoCV(
                alphas=self.hyperParams,
                precompute=False,
                max_iter=1000,
                tol=1e-3,
            ...
            self.policyParams = scipy.linalg.lstsq(
                designMatrix, regressionTargets, cond=1e-9
            )[0]
            print("CustomPolicy:train [INFO] %s " % self.modelType, flush=True)

        else:
            print(
                "CustomPolicy:train [ERR] %s not supported." % self.modelType,
                flush=True,
            )
            sys.exit(0)

        print(
            "CustomPolicy:train [INFO] Created %s predictor using dataset %s."
            % (self.modelType, dataset.name),
            flush=True,
        )
...
class DeterministicPolicy(Policy):
    ...
    def train(self, dataset, feature_list, exponential_gains):
        ...
        if self.modelType == "tree":
            ...
            self.tree = treeCV.best_estimator_
            print(
                "DeterministicPolicy:train [INFO] %s Best depth: %d Exponentiated Gains?"
                % (self.modelType, treeCV.best_params_["max_depth"]),
                exponential_gains,
                flush=True,
            )
            ...
            self.policyParams = cv.coef_
            print(
                "DeterministicPolicy:train [INFO] %s CVAlpha: %f Exponentiated Gains?"
                % (self.modelType, cv.alpha_),
                exponential_gains,
                flush=True,
            )
            ...
            self.policyParams = scipy.linalg.lstsq(
                flatFeatures, flatRelevances, cond=1e-9
            )[0]
            print(
                "DeterministicPolicy:train [INFO] %s Exponentiated Gains?"
                % self.modelType,
                exponential_gains,
                flush=True,
            )
            ...
            print(
                "DeterministicPolicy:train [ERR] %s not supported." % self.modelType,
                flush=True,
            )
            sys.exit(0)

        print(
            "DeterministicPolicy:train [INFO] Created %s predictor using dataset %s. Features:"
            % (self.modelType, dataset.name),
            feature_list,
            flush=True,
        )
    ...
    def predictAll(self, ranking_size):
        ...
        print(
            "DeterministicPolicy:predictAll [INFO] Generated all predictions for %s using %s."
    ...
    def savePolicy(self, file_path):
        ...
        outFile.close()
        print(
            "DeterministicPolicy:savePolicy [INFO] Saved trained policy at ",
            outputFileName,
            flush=True,
        )
    ...
    def loadPolicy(self, file_name):
        ...
        print(
            "DeterministicPolicy:loadPolicy [INFO] Loaded trained policy from ",
            file_name,
            flush=True,
        )
    ...
    def createFilteredDataset(self, num_allowed_docs):
        ...
        newDataset.features = numpy.nan * numpy.ones(
            (oldNumQueries, num_allowed_docs, oldNumFeatures), dtype=numpy.float32
        )
        newDataset.docsPerQuery = numpy.clip(
            self.dataset.docsPerQuery, 0, num_allowed_docs
        )
...
class StochasticPolicy(Policy):
    ...
    def __init__(self, dataset, temperature, det_policy):
        ...
        self.documentProbabilities = (
            None
        )  # Internally used by StochasticPolicy:predict.
        ...
        if det_policy is not None:
            print(
                "StochasticPolicy:init [INFO] Deterministic policy: %s Temperature:"
                % det_policy.name(),
                temperature,
                flush=True,
            )
            ...
            print(
                "StochasticPolicy:init [INFO] Deterministic policy: None Temperature:",
                temperature,
                flush=True,
            )
    ...
    def predict(self, query_id, ranking_size):
        ...
        producedRanking[0:validDocs] = numpy.random.choice(
            allowedDocs, size=validDocs, replace=False, p=samplingProbability
        )
        return producedRanking
    ...
    def explorationHelper(self, query_id):
        ...
        uniformDistribution = (
            numpy.ones(numAllowedDocs, dtype=numpy.float32) / numAllowedDocs
        )
        ...
        if self.detPolicy is None:
            ...
            if self.temperature <= 0:
                ...
                numFeatures = numpy.shape(self.dataset.features)[2]
                allDocFeatures = numpy.zeros(
                    (numAllowedDocs, numFeatures), dtype=numpy.float32
                )
                currentFeatures = self.dataset.features[query_id, 0:numAllowedDocs, :]
                allDocFeatures[:, self.detPolicy.featureList] = currentFeatures[
                    :, self.detPolicy.featureList
                ]
                allDocScores = None
                if self.detPolicy.modelType != "tree":
                    allDocScores = numpy.dot(
                        allDocFeatures, self.detPolicy.policyParams
                    )
                else:
    ...
    def gammaHelper(self, i, ranking_size):
        ...
        if self.temperature > 0:
            slates = numpy.zeros(
                tuple([numAllowedDocs for p in range(validDocs)]), dtype=numpy.float32
            )
        ...
        gamma = numpy.zeros(
            (numAllowedDocs * validDocs, numAllowedDocs * validDocs),
        ...
        for p in range(validDocs):
            ...
            if self.temperature > 0:
                currentMarginals = numpy.sum(
                    slates,
                    axis=tuple([q for q in range(validDocs) if q != p]),
                    dtype=numpy.longdouble,
                )
            else:
                currentMarginals = (
                    numpy.ones(numAllowedDocs, dtype=numpy.longdouble) / numAllowedDocs
                )
            gamma[currentStart:currentEnd, currentStart:currentEnd] = numpy.diag(
                currentMarginals
            )
        ...
        for p in range(validDocs):
            for q in range(p + 1, validDocs):
                ...
                pairMarginals = None
                if self.temperature > 0:
                    pairMarginals = numpy.sum(
                        slates,
                        axis=tuple([r for r in range(validDocs) if r != p and r != q]),
                        dtype=numpy.longdouble,
                    )
                else:
                    pairMarginals = numpy.ones(
                        (numAllowedDocs, numAllowedDocs), dtype=numpy.longdouble
                    ) / (numAllowedDocs * (numAllowedDocs - 1))
                    numpy.fill_diagonal(pairMarginals, 0)

                gamma[
                    currentRowStart:currentRowEnd, currentColumnStart:currentColumnEnd
                ] = pairMarginals
                gamma[
                    currentColumnStart:currentColumnEnd, currentRowStart:currentRowEnd
                ] = pairMarginals.T
    ...
    def setupExploration(self, ranking_size):
        ...
        self.documentProbabilities = -numpy.ones(
            (numQueries, numDocs), dtype=numpy.float32
        )
        ...
        responses = Parallel(n_jobs=-2, verbose=50)(
            delayed(self.gammaHelper)(i, ranking_size) for i in range(numQueries)
        )
        ...
        print("")
        print(
            "StochasticPolicy:setupExploration [INFO] RankingSize: %d" % ranking_size,
            flush=True,
        )
    ...
    def serializeGammas(self, file_path, ranking_size):
        ...
        outFile.close()
        print(
            "StochasticPolicy:serializeGammas [INFO] Saved computed gammas", flush=True
        )
    ...
    def loadGammas(self, file_name):
        ...
        print(
            "StochasticPolicy:loadGammas [INFO] Loaded precomputed gammas from %s"
            % file_name,
            flush=True,
        )
...
if __name__ == "__main__":
    ...
    print(
        "New dataset NumQueries, NumDocs, NumFeatures, [Min,Max]NumDocs: ",
        numpy.shape(newData.features),
        numpy.min(newData.docsPerQuery),
        numpy.max(newData.docsPerQuery),
    )
    ...
    print(
        "After load, are predicted rankings same?",
        numpy.allclose(predictedRankings, predictedRankings2),
        flush=True,
    )
    ...
    print(
        "Uniform Stochastic ranking: ", uniformNoRepLogger.predict(queryID, rankingSize)
    )
    print(
        "NonUniform Stochastic ranking: ",
        nonUniformNoRepLogger.predict(queryID, rankingSize),
    )
    print(
        "Parametrized Stochastic ranking: ",
        paramNoRepLogger.predict(queryID, rankingSize),
    )

    uniformNoRepLogger.serializeGammas("./", rankingSize)
    copyLogger = StochasticPolicy(newData, 0, None)
    copyLogger.loadGammas(
        "./" + uniformNoRepLogger.name() + "_gamma" + str(rankingSize) + ".pkl"
    )
