...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        metavar="D",
        type=bool,
        help="Materializes a dataset in vw-compatible format",
        default=False,
    )
    parser.add_argument(
        "--output_dir",
    ...
    if args.max_docs is not None:
        ...
        print(
            "[ERROR] Must specify a filterer for optimization expt with max_docs > 0",
            flush=True,
        )
        sys.exit(0)
    ...
    exploredRankingFeatures = numpy.empty(
        (numInstances, numFeatures * args.length_ranking), dtype=numpy.float32
    )
    exploredValues = numpy.empty(numInstances, dtype=numpy.float32)

    relevances = numpy.zeros(
        (numQueries, args.length_ranking, numDocs), dtype=numpy.float32
    )
    ...
    for i in range(numInstances):
        ...
        samplingProbabilities = (
            expt.logger.policy.documentProbabilities[currentQuery, 0:numAllowedDocs]
        ).copy()
        invPropensity = 1.0
        ...
        for j in range(args.length_ranking):
            ...
            invPropensity *= (
                samplingProbabilities.sum(dtype=numpy.longdouble)
                / samplingProbabilities[currAction]
            )
            samplingProbabilities[currAction] = 0.0

        inversePropensities[i] = invPropensity

        estimatedPhi = numpy.dot(
            expt.logger.policy.gammaInverses[currentQuery], posRelVector
        )
        ...
        if args.dump_vw and (numInstances - i) <= 1000:
            ...
            for eachRanking in itertools.permutations(range(numAllowedDocs), validDocs):
                ...
                if matched:
                    prefixStr += (
                        ":"
                        + str(1.0 - (currentValue / 750.0))
                        + ":"
                        + str(1.0 / invPropensity)
                    )
                    ...
                    for m in range(numFeatures):
                        if allFeatures[eachRanking[k], m] != 0.0:
                            featString += (
                                str(k * numFeatures + m)
                                + ":"
                                + str(allFeatures[eachRanking[k], m])
                                + " "
                            )
    ...
    currentPolicy = Policy.CustomPolicy(
        banditDataset, numFeatures, args.length_ranking, "tree", True
    )
    currentPolicy.train(banditDataset, currentRelevances)

    print(
        "*** SEEN LOGS: ",
        numpy.mean(exploredValues, dtype=numpy.longdouble),
        flush=True,
    )

    folds = sklearn.cross_validation.LabelKFold(exploredQueries, n_folds=5)
    estimator = Estimators.DirectEstimator(
        expt.logger.policy, "tree", expt.rankingSize, numFeatures
    )
    ...
    for i in range(numQueries):
        ...
        for eachRanking in itertools.permutations(range(allowedDocs), validDocs):
            ...
            if args.dump_vw:
                ...
                vwFile2.write(
                    str(currId) + " " + str(1.0 - (metricScore / 750.0)) + "\n"
                )
                prefixStr = str(currId) + " | "
                featString = ""
                for k in range(args.length_ranking):
    ...
    print(
        "*** DIRECT: ", numpy.mean(directRevenues, dtype=numpy.longdouble), flush=True
    )
    print(
        "*** SUPERVISED: ",
        numpy.mean(supervisedRevenues, dtype=numpy.longdouble),
        flush=True,
    )
